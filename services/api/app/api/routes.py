...
from app.domain.schemas import JobListResponse, ResultResponse, SubmitExperimentRequest, SubmitExperimentResponse
...
async def get_job_result(
    ...
    if result is None:
        correlation_id = get_correlation_id()
        raise HTTPException(status_code=404, detail=not_found(correlation_id, "result").model_dump())
    return ResultResponse(result=result)


@router.get("/v1/comparison")
