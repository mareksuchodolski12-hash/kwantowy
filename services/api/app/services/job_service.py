...
from app.db.models import JobModel, ResultModel
...
class JobService:
    ...
    async def comparison(self) -> dict[str, dict[str, float | int]]:
        provider_stats: dict[str, dict[str, float | int]] = {}
        rows = await self.session.execute(
            select(JobModel.provider, ResultModel.duration_ms).join(ResultModel, ResultModel.job_id == JobModel.id)
        )
        for provider, duration_ms in rows:
            entry = provider_stats.setdefault(provider, {"runs": 0, "avg_duration_ms": 0.0})
            runs = int(entry["runs"]) + 1
            avg = float(entry["avg_duration_ms"])
            entry["runs"] = runs
            entry["avg_duration_ms"] = ((avg * (runs - 1)) + float(duration_ms)) / runs
